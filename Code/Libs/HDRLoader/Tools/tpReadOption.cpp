...
/*!
  ...
  \param pszValidOpts : string of valid, case-sensitive option characters, a
  ...
  \return If valid option is found, the character value of that option is
          returned, and *ppszParam points to the parameter if given, or is NULL
          if no param.  If standalone parameter (with no option) is found, 1 is
          returned, and *ppszParam points to the standalone parameter.  If
          option is found, but it is not in the list of valid options, -1 is
          returned, and *ppszParam points to the invalid argument.  When end of
          argument list is reached, 0 is returned, and *ppszParam is NULL.
*/
int tpReadOption::getoption (int argc, char** argv, char* pszValidOpts,
			     char** ppszParam)
{
  static int iArg = 1;
  int chOpt;
